...
VOICE = "/usr/local/share/piper-voices/en_GB-semaine-medium.onnx"
...
def _voice_sample_rate(voice_path):
    """Piper's --output-raw rate is model-specific (e.g. 22050Hz medium vs 16000Hz low quality)
    -- read it from the voice's own companion .json rather than assuming, so switching VOICE can't
    silently reintroduce a pitch/speed bug in the sox resample step below (caught 2026-07-15
    switching to southern_english_female-low, a 16000Hz voice, while the script still assumed
    22050Hz)."""
    with open(f"{voice_path}.json") as f:
        return str(json.load(f)["audio"]["sample_rate"])


VOICE_SAMPLE_RATE = _voice_sample_rate(VOICE)
...
def speak(announcement_type, maritime_text, temp_sentence):
    ...
    sox = subprocess.Popen(
        ...
        ["sox", "-G", "-t", "raw", "-r", VOICE_SAMPLE_RATE, "-e", "signed-integer", "-b", "16", "-c", "1", "-",
